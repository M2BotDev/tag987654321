...
class Add:
    ...
    async def add(self,ctx, amount=None, person=None):
        message = ctx.message
        ...
        try:
            ...
            print(therror)
            await message.channel.send("Please enter a valid amount. (Ex. $add 2000 @MyFriend#1123")
            return
        try:
        ...
        if str(message.guild.id) in data:
            ...
            await message.channel.send(f"Added ``{amount}`` to ``{person}``'s balance. <:nbaghost:368751631455748096> They now have ``{data[str(message.guild.id)]['tickets-storage'][str(person.id)]['credits']}`` tickets.")
            ...
            if str(message.guild.id) in jsondatafile:
                try:
                    ...
                    action = f"Added {amount} to {person}'s balance."
